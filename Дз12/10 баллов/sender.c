...
int main(int argc, char *argv[]) {
    ...
    char sendString[MAXSENDSTRING];                 /* String to multicast */
    ...
    if ((argc < 3) || (argc > 4)) {        /* Test for correct number of parameters */
        fprintf(stderr,"Usage:  %s <Multicast Address> <Port> [<TTL>]\n",
                 argv[0]);
        exit(1);
    }
    ...
    if (argc == 4)                     /* Is TTL specified on command-line? */
        multicastTTL = atoi(argv[3]);  /* Command-line specified TTL */
    else
